...
for option in providedOptions:
    ...
    if bool(isPyType and optionKey in pyOptionKeys) or bool(
        isSpecType and optionKey in specOptionKeys
    ):
...
if supportedOptions:
    set_output("supported_options", " ".join(supportedOptions))
    set_annotation(
...
set_output("spec_path", os.path.basename(specPathNew) + ".spec")


## ---------------------- Spec options ---------------------- ##
isOptionSet = isSpecType and bool(spec_options)
set_output("supported_spec_options", f"-- {spec_options}" if isOptionSet else "")
if isOptionSet:
    set_annotation(f"Custom Spec Options: {spec_options}", "Pyinstaller Options")
